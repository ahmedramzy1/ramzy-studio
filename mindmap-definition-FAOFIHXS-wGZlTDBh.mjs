...
import { $n as n, Qn as r, Rn as i, ar as a, bn as o, cr as s, kn as c, pn as l, r as u, t as d, tr as f, xn as p } from "./chunk-J7OUQ5F2-BUGcDHyW.mjs";
import { t as m } from "./chunk-XXDRQBXY-DYEdPnIN.mjs";
import { t as h } from "./chunk-KBJHAD2P-iCC8cYwM.mjs";
...
var g = (function() {
	var e = /* @__PURE__ */ s(function(e, t, n, r) {
		...
		return n;
	}, "o"), t = [1, 4], n = [1, 13], r = [1, 12], i = [1, 15], a = [1, 16], o = [1, 20], c = [1, 19], l = [
		6,
		7,
		8
	...
	return s(y, "Parser"), y.prototype = v, v.Parser = y, new y();
})();
g.parser = g;
var _ = g, v = 12, y = {
	DEFAULT: 0,
	...
	addNode(e, t, n, r) {
		...
		let s = p(), c = s.mindmap?.padding ?? l.mindmap.padding;
		switch (r) {
		...
		if (d) d.children.push(u), this.nodes.push(u);
		else if (o) this.nodes.push(u);
		else throw Error(`There can be only one root. No parent could be found for ("${u.descr}")`);
	}
	getType(e, t) {
	...
	decorateNode(e) {
		if (!e) return;
		let t = p(), n = this.nodes[this.nodes.length - 1];
	...
	getData() {
		let e = this.getMindmap(), n = p(), r = c().layout !== void 0, i = n;
		if (r || (i.layout = "cose-bilkent"), !e) return {
			nodes: [],
			edges: [],
			config: i
		};
		a.debug("getData: mindmapRoot", e, n), this.assignSections(e);
		let o = [], s = [];
		this.flattenNodes(e, o), this.generateEdges(e, s), a.debug(`getData: processed ${o.length} nodes and ${s.length} edges`);
...
}, x = { draw: /* @__PURE__ */ s(async (e, t, n, r) => {
	a.debug("Rendering mindmap diagram\n" + e);
	let i = r.db, s = i.getData(), c = m(t, s.config.securityLevel);
	if (s.type = r.type, s.layoutAlgorithm = d(s.config.layout, { fallback: "cose-bilkent" }), s.diagramId = t, !i.getMindmap()) return;
	s.nodes.forEach((e) => {
	...
	let { themeVariables: f } = o(), { useGradient: p, gradientStart: g, gradientStop: _ } = f;
	if (p && g && _) {
		let e = c.attr("id"), t = c.append("defs").append("linearGradient").attr("id", `${e}-gradient`).attr("gradientUnits", "objectBoundingBox").attr("x1", "0%").attr("y1", "0%").attr("x2", "100%").attr("y2", "0%");
	...
	h(c, s.config.mindmap?.padding ?? l.mindmap.padding, "mindmapDiagram", s.config.mindmap?.useMaxWidth ?? l.mindmap.useMaxWidth);
}, "draw") }, S = /* @__PURE__ */ s((e) => {
	let { theme: t, look: i } = e, a = "";
	for (let t = 0; t < e.THEME_COLOR_LIMIT; t++) e["lineColor" + t] = e["lineColor" + t] || e["cScaleInv" + t], f(e["lineColor" + t]) ? e["lineColor" + t] = n(e["lineColor" + t], 20) : e["lineColor" + t] = r(e["lineColor" + t], 20);
	...
	return a;
}, "genSections"), C = /* @__PURE__ */ s((e, t, n) => {
	let r = "";
	for (let i = 0; i < e; i++) r += `
