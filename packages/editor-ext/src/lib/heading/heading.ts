...
export const Heading = TiptapHeading.extend<TiptapHeadingOptions>({
  /**
   * Portfolio navigation may use a shorter authored label than the visible
   * heading. Keeping it on the heading node means document structure and
   * navigation can never drift into two independent content models.
   */
  addAttributes() {
    return {
      ...(this.parent?.() ?? {}),
      navigationLabel: {
        default: null,
        parseHTML: (element) =>
          element.getAttribute("data-navigation-label") || null,
        renderHTML: (attributes) => {
          const navigationLabel =
            typeof attributes.navigationLabel === "string"
              ? attributes.navigationLabel.trim()
              : "";
