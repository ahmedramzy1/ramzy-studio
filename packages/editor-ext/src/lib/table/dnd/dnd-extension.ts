...
class TableHandlePluginSpec implements PluginSpec<TableHandleState> {
  ...
  private _pointerMove = (view: EditorView, event: PointerEvent) => {
    ...
    // Keep the hovered table/axis stable across its perimeter gutter. Using
    // the text selection here can jump a grip to a different row or table
    // just as the pointer reaches it.
    const hoveredDOM = current?.hoveringCell
      ? view.nodeDOM(current.hoveringCell.cellPos)
      : null;
    const tableRect =
      hoveredDOM instanceof HTMLElement
        ? hoveredDOM.closest('table')?.getBoundingClientRect()
        : null;
    if (
      tableRect &&
      event.clientX >= tableRect.left - 24 &&
      event.clientX <= tableRect.right + 24 &&
      event.clientY >= tableRect.top - 24 &&
      event.clientY <= tableRect.bottom + 24
    )
      return;
